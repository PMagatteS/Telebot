...
def get_updates(api_url, offset = None):
    updates_url = api_url + 'getUpdates?timeout=100'
    if offset:
            ...
            if len(updates['result']) > 0:
                ...
                for update in updates.get('result'):
                    if update.get('message').get('text'):
                        if '/stop' in update.get('message').get('text'):
                            get_updates(api_url, offsets)
                            getAllfiles = False
...
if os.path.exists('commandlist.json'):
    with open('commandlist.json') as f:
